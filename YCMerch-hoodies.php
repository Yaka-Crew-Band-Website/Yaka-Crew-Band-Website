...
$stmt = $pdo->query("SELECT * FROM hoodies");
$hoodies = $stmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
.dropdown li a:hover {
  ...
  <meta charset="UTF-8" />
  <title>Band Hoodies | YAKA Crew</title>
...
document.addEventListener('DOMContentLoaded', updateMerchCartCount);
    ...
    <h1 class="section-title">BAND HOODIES</h1>

    <section class="grid">
  <?php foreach ($hoodies as $row): ?>
        <article class="card">
          <div class="thumb">
            <?php if (!empty($row['image_black_front']) && file_exists("uploads/YCMerch-uploads/" . $row['image_black_front'])): ?>
              <img src="uploads/YCMerch-uploads/<?= htmlspecialchars($row['image_black_front']) ?>" alt="<?= htmlspecialchars($row['name']) ?> Black Front" />
            <?php else: ?>
      ...
      <a href="YCMerch-producthoodies.php?id=<?= htmlspecialchars($row['id']) ?>">
