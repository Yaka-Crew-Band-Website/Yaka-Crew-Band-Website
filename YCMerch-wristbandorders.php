<?php
// YCMerch-wristbandorders.php: Order summary for wristbands only (PDO version)
require_once __DIR__ . '/YCdb_connection.php';

// Always ensure the band_orders table exists
$pdo->exec("CREATE TABLE IF NOT EXISTS band_orders (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $band_id = isset($_POST['band_id']) ? (int)$_POST['band_id'] : (isset($_POST['product_id']) ? (int)$_POST['product_id'] : (isset($_POST['id']) ? (int)$_POST['id'] : 0));
  $quantity = isset($_POST['quantity']) ? (int)$_POST['quantity'] : 1;
  $price = isset($_POST['price']) ? floatval($_POST['price']) : (isset($_POST['product_price']) ? floatval($_POST['product_price']) : 0.00);

  // Fetch band details
  $band_name = '';
  $stmt = $pdo->prepare("SELECT name FROM wristband WHERE id = ? LIMIT 1");
  $stmt->execute([$band_id]);
  $row = $stmt->fetch();
  if ($row) {
    $band_name = $row['name'];
  } else {
    die("Wristband not found.");
  }

  $total_cost = $price * $quantity;

  $stmt = $pdo->prepare("INSERT INTO band_orders (band_id, band_name, quantity, price, total_cost) VALUES (?, ?, ?, ?, ?)");
  $stmt->execute([$band_id, $band_name, $quantity, $price, $total_cost]);
  $order_id = (int)$pdo->lastInsertId();

  // Redirect to self with GET params to prevent resubmission
  header("Location: " . $_SERVER['PHP_SELF'] . "?order_id=$order_id&band_id=$band_id&quantity=$quantity&price=$price");
  exit;
}

// GET: Show order summary only (no DB insert)

if (isset($_GET['order_id'], $_GET['band_id'], $_GET['quantity'])) {
  ...
  require_once __DIR__ . '/YCdb_connection.php';
  if ($band_id > 0) {
    $stmt = $pdo->prepare("SELECT name, price FROM wristband WHERE id = ? LIMIT 1");
    $stmt->execute([$band_id]);
    $row = $stmt->fetch();
    if ($row) {
      $band_name = $row['name'];
...
<title>Band Order Summary - YAKA Crew</title>
<style>
...
<div class="container" role="main" aria-label="Band Order Summary">
  <!-- ORDER SUMMARY -->
  <section class="card" aria-labelledby="order-summary-title" tabindex="0">
    ...
    <div class="order-details" role="region" aria-live="polite" aria-atomic="true">
      ...
      <p><strong>Band ID:</strong> <span><?= htmlspecialchars($band_id) ?></span></p>
      <p><strong>Band Name:</strong> <span><?= htmlspecialchars($band_name) ?></span></p>
